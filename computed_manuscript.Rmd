...
Rather than analyzing those data and then copying the results into our manuscript, we can use the programming language `R` to do that in the manuscript itself. Simply enclose the code inside backticks, with the letter `r`. For instance, we could calculate the circumference and area of a circle (written in $\LaTeX$): 

$$A = \pi r^2, C = 2 \pi r$$

Plugging in the radius *r* = **`r r`**, you could write "A = `` `r
pi * r^2` `` and C = `` `r
